...
class Check extends REST_Controller {
	...
	public function is_email_availaible_get()
	{
		...
		$result = $this->user_model2->is_email_availaible($this->get('email'));

		if ($result > 0)  
			$this->response(array('error' => $this->lang->line("check_email_already_used")), 403);
		else 
	...
	public function is_username_availaible_get()
	{
		...
		$result = $this->user_model2->is_username_available($this->get('pseudo'));

		if ($result > 0) 
			$this->response(array('error' => $this->lang->line("check_pseudo_already_used")), 403);
		else
